    ...
    def run(self):
        try:
            ...
            while True:
                ...
                client_socket.connect((new_endpoint_address, new_endpoint_port))
                print(f"Connected to {new_endpoint_address}:{new_endpoint_port}")
                subprocess.run(f'wg syncconf {WIREGUARD_INTERFACE_NAME} <(wg-quick strip {WIREGUARD_INTERFACE_NAME})')
